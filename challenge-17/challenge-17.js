(function() {
  ...
  console.log(
    text.replace(/Manuel Marques de Sousa/g, "Giovani de Sousa Leite")
  );
  ...
  console.log(text.replace(/O Centauro de Luvas/g, text => text.toUpperCase()));
  ...
  var regexDate = /(\d{2}) de (\w+) de (\d{4})/g;
  ...
  /*
    ...
    */
  console.log("\nReplace de datas:");
  var replaceDate = function replaceDate(_, day, month, year) {
    return `${day}/${getMonthNumber(month)}/${year}`;
  };

  console.log(text.replace(/(\d{2}) de (\w+) de (\d{4})/g, replaceDate));
})();
